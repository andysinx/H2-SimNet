...
def compute_redundancy_matrix(ps_single, feature_base, anomalous_files, position, num_runs=5):
    ...
    feature_cols_F = fixed_F  # solo F

    for _ in range(num_runs):

        folds_F, test_loader_F = create_unsupervised_dataset(
        ...
        sample_X, _ = next(iter(folds_F[0][0]))
        ...
        for train_loader, val_loader in folds_F:
            model = train_autoencoder(model, train_loader, val_loader, num_epochs=50)

        LF_mean, _ = compute_LF(model, test_loader_F)
        baseline_LF_runs.append(LF_mean)

    L_F = {
        "mean": np.mean(baseline_LF_runs),
        "std": np.std(baseline_LF_runs),
        "var": np.var(baseline_LF_runs)
    }

    results["F"] = L_F

    for ps_i in ps_single:

        feature_cols = fixed_F + [ps_i]
...
if __name__ == "__main__":
    ...
    # ===============================
    fixed_F = [f"MFS{i}" for i in range(1, 5)]
    ps_single = [f"PS{i}" for i in range(1, 13)]
    ...
    for ps_i, stats in redundancy_matrix.items():
