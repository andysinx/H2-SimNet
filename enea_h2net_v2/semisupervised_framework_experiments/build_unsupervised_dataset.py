...
# --- Sliding windows ---
def create_windows(df, feature_cols, window_size=50, step=1):
    data = df[feature_cols].values
    return np.array([data[i:i+window_size] for i in range(0, len(data)-window_size+1, step)], dtype=np.float32)
...
def create_unsupervised_dataset(anomalous_file, window_size=50, step=1, derivatives=False,
    ...
    # --- K-Fold training/validation (train solo normali, validation con ~5% anomalie) ---
    folds = []
    if k_fold is not None:
        kf = KFold(n_splits=k_fold, shuffle=True, random_state=42)
        X_val_anom, y_val_anom = load_anomalous_data(anomalous_file, feature_cols,
                                                    window_size, step,
                                                    position=position,
        ...
        for train_idx, val_idx in kf.split(X_trainval):
            ...
            y_train = np.zeros(len(train_idx))  # solo normali

            # Calcoliamo 5% anomalie sul fold di validazione
            num_val = len(X_val)
            desired_anom_count_val = max(1, int(num_val * max_anom_ratio))
            if len(X_val_anom) >= desired_anom_count_val:
                selected_idx_val = np.random.choice(len(X_val_anom), desired_anom_count_val, replace=False)
            else:
                selected_idx_val = np.random.choice(len(X_val_anom), desired_anom_count_val, replace=True)

            X_val_anom_fold = X_val_anom[selected_idx_val]
            y_val_anom_fold = y_val_anom[selected_idx_val]

            # Concatenate anomalie al validation fold
            X_val_fold = np.concatenate([X_val, X_val_anom_fold], axis=0)
            y_val_fold = np.concatenate([np.zeros(len(X_val), dtype=int), y_val_anom_fold], axis=0)

            perm = np.random.permutation(len(y_val_fold))
